...
WHL_DEST ?= ${BUILD_ROOT}
WHL_FNAME ?= ${PYPI_PACKAGE_NAME}-${PYPI_PACKAGE_VERSION}-cp35-cp35m-linux_armv7l.whl
...
BUILD_CMD ?= /usr/local/bin/pip3 --disable-pip-version-check install wheel && /usr/local/bin/pip3 -v --disable-pip-version-check wheel --no-deps ${PYPI_PACKAGE_NAME}==${PYPI_PACKAGE_VERSION}
